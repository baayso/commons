 ...
 * @since 1.0.0
 */
public class LoadPackageAndAssignableTypeClasses {
     ...
     * @param filter         指定扫描包中含有特定类型的bean，支持多个类型
     *
     * @since 1.0.0
     */
    public LoadPackageAndAssignableTypeClasses(String[] packagesToScan, Class<?>... filter) {
        ...
        if (filter != null) {
            for (Class<?> clazz : filter) {
                this.typeFilters.add(new AssignableTypeFilter(clazz));
            }
        }
    }
    ...
    /** 检查当前扫描到的class是否是指定的类型（包括子类或实现类） */
